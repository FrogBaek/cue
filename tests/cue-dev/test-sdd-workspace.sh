#!/usr/bin/env bash
# Tests for the SDD workspace: sdd-workspace resolves a self-ignoring, PER-PLAN
# working-tree directory for SDD artifacts, and the SDD scripts write into their
# plan's directory.
#
# This suite lived under tests/claude-code/, a directory tools/run-tests.sh
# excluded because its other suites drove real LLM sessions, and therefore never
# ran — it is plain shell. Three scripts had their only coverage parked where
# nothing looked at it. That directory is gone now, and the lesson it left is that
# an exclusion is by directory: whatever is put behind one runs nowhere and says
# nothing about it.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
SDD_SCRIPTS="$REPO_ROOT/plugins/cue-dev/skills/using-cue/scripts"
...
main() {
    ...
    # macOS mktemp lives under /var -> /private/var), then normalize through
    # pwd: on Git Bash --show-toplevel yields `C:/...` while the scripts
    # (and `pwd`) yield `/c/...`, and mixing the two fails only on Windows.
...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.cue/dev/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.cue/dev/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.cue/dev/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.cue/dev/sdd/.gitignore" && "$(cat "$repo/.cue/dev/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .cue/dev/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .cue/dev/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.cue/dev/sdd/plan-a/task-1-brief.md" ]]; then
    ...
    case "$rp_path" in
        "$repo/.cue/dev/sdd/plan-a/review-"*.diff)
    ...
    wt_root="$(cd "$(cd "$wt" && git rev-parse --show-toplevel)" && pwd)"
    wt_dir="$(cd "$wt" && "$SDD_SCRIPTS/sdd-workspace" plan-a.md)"
    if [[ "$wt_dir" == "$wt_root/.cue/dev/sdd/plan-a" && "$wt_dir" != "$dir_a" ]]; then
    ...
    # --- cue-dev's own layout: every plan is called plan.md -------------------
    #
    # The basename was the whole of the slug, so two items in one checkout both
    # resolved to `.cue/dev/sdd/plan/` and the second item's task-1-brief.md
    # landed on top of the first's. Per-plan scoping that scopes nothing is worse
    # than none: it reads as a guarantee. The KEY is the parent directory.
    local key_a key_b
    ( cd "$repo" && mkdir -p .cue/dev/PROJ-1 .cue/dev/PROJ-2 \
        && printf '### Task 1: a\n' > .cue/dev/PROJ-1/plan.md \
        && printf '### Task 1: b\n' > .cue/dev/PROJ-2/plan.md )
    key_a="$(cd "$repo" && "$SDD_SCRIPTS/sdd-workspace" .cue/dev/PROJ-1/plan.md)"
    key_b="$(cd "$repo" && "$SDD_SCRIPTS/sdd-workspace" .cue/dev/PROJ-2/plan.md)"
    if [[ "$key_a" == */.cue/dev/sdd/PROJ-1 && "$key_b" == */.cue/dev/sdd/PROJ-2 ]]; then
        pass "a plan.md takes its workspace name from the KEY, not from 'plan'"
    else
        fail "a plan.md takes its workspace name from the KEY, not from 'plan'"
        echo "    a: $key_a"
        echo "    b: $key_b"
    fi
